...
//#endregion
//#region src/google-workspace.members.generated.ts
const GOOGLE_WORKSPACE_MEMBER_INDEX = [
	{
...
//#endregion
//#region src/google-workspace.requests.generated.ts
const GOOGLE_WORKSPACE_REQUESTS = {
	"drive": {
...
const GOOGLE_WORKSPACE_APPLICATION_CARD = {
	...
	summary: "Drive, Gmail, Calendar, Docs, Sheets, Slides, Chat, and People with native Drive transfers."
};
const GOOGLE_WORKSPACE_AUTH_REQUIREMENTS = {
...
const GOOGLE_WORKSPACE_APPLICATION_FACE = {
...
function installGoogleWorkspace(target, transport, workspace, services = GOOGLE_WORKSPACE_REQUESTS) {
	...
	const accessToken = (interactive) => {
		if (!token) token = transport.__token(interactive).then((authorization) => {
			if (!authorization.ok) throw new Error(`Google Workspace authentication is required; call await ctx.applications["${GOOGLE_WORKSPACE_APPLICATION_ID}"].authorize()`);
	...
	const readBoundedError = async (response) => {
	...
	const requestUrl = (service, method, input, path = method.path) => {
		const used = /* @__PURE__ */ new Set();
		const renderedPath = path.replace(/\{(\+?)([^}]+)\}/gu, (_match, reserved, name) => {
			const value = input[name];
			if (value === void 0 || value === null || value === "") throw new Error(`Google Workspace request requires ${name}`);
			used.add(name);
			const stringValue = String(value);
			return reserved ? stringValue.split("/").map(encodeURIComponent).join("/") : encodeURIComponent(stringValue);
		});
		const url = new NativeURL(renderedPath, service.baseUrl);
		for (const [name, parameter] of Object.entries(method.parameters)) {
			if (used.has(name)) continue;
			const value = input[name];
			if (parameter.required && (value === void 0 || value === null || value === "")) throw new Error(`Google Workspace request requires ${name}`);
			if (value === void 0 || value === null) continue;
			used.add(name);
			if (parameter.location !== "query") continue;
			if (parameter.repeated && Array.isArray(value)) for (const item of value) url.searchParams.append(name, String(item));
			else url.searchParams.append(name, String(value));
		}
		for (const name of Object.keys(input)) if (name !== "requestBody" && !used.has(name)) throw new Error(`Google Workspace request does not accept ${name}`);
		return url.href;
	};
	const send = async (service, method, input) => {
		const body = method.request && input.requestBody !== void 0 ? nativeJsonStringify(input.requestBody) : void 0;
	...
	const invoke = async (service, method, input = {}) => {
		const response = await send(service, method, input);
		if (response.status === 204 || response.headers.get("content-length") === "0") return void 0;
		if ((response.headers.get("content-type") ?? "").includes("json")) return nativeJsonParse(await response.text());
		return response.blob();
	};
	const buildResource = (service, resource) => {
		const client = {};
		for (const [name, method] of Object.entries(resource.methods)) client[name] = (input) => invoke(service, method, input);
		for (const [name, child] of Object.entries(resource.resources)) client[name] = buildResource(service, child);
		return client;
	};
	const namespace = { authorize: async () => {
	...
	if (!driveFiles || typeof driveFiles !== "object") throw new Error("Google Drive files resource is missing");
	...
	Object.assign(driveFiles, {
		async upload(input) {
			...
			if (!(file instanceof NativeFile)) throw new Error("drive.files.upload requires a native File");
		...
		async downloadToWorkspace(input) {
			const { path, sha256, ...requestInput } = input;
			const response = await send(driveService, getFile, {
				...requestInput,
				alt: "media"
			});
			if (!(response.body instanceof NativeReadableStream)) throw new Error("Google Drive download returned an invalid response body");
...
//#endregion
export { GOOGLE_WORKSPACE_APPLICATION_CARD, GOOGLE_WORKSPACE_APPLICATION_FACE, GOOGLE_WORKSPACE_APPLICATION_ID, GOOGLE_WORKSPACE_AUTH_REQUIREMENTS, GOOGLE_WORKSPACE_MEMBER_INDEX, GOOGLE_WORKSPACE_REQUESTS, installGoogleWorkspace };