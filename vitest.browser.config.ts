...
const googleWorkspaceBoundary: Plugin = {
  ...
  configureServer(server) {
    server.middlewares.use(async (request, response, next) => {
      ...
      if (providerPath === "/upload/drive/v3/files" && request.method === "POST") {
        ...
        const metadata: unknown = JSON.parse(body.subarray(metadataStart, metadataEnd).toString());
        if (
      ...
      if (
        ...
        response.end(GOOGLE_WORKSPACE_TEST_FILE_BYTES);
        return;
      }
